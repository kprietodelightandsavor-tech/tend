...
const NATURE_DAYS = {
  Monday:    { step: "Read",    label: "Nature Lore Reading",       getInstruction: (t) => t ? "Read aloud from " + t.read + ". This week: " + t.subject + "." : "Read aloud from The Year Round by C.J. Hylander." },
  Tuesday:   { step: "Observe", label: "Nature Walk",              getInstruction: (t) => t?.observe || "Go outside and observe nature with fresh eyes." },
  Wednesday: { step: "Record",  label: "Consider the Lilies Entry", getInstruction: ()  => "Words or sentences, a sketch, and watercolor. Let the page be a living record of what you noticed." },
  Thursday:  { step: "Finish",  label: "Continue Reading",         getInstruction: (t) => t ? "Finish any remaining reading on " + t.subject + ", or revisit Tuesday's walk through narration." : "Finish this week's nature lore reading." },
...
function NatureStudyCard({ onNavigate, today }) {
  const [current] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("tend_nature_current") || "null");
      if (saved?.subject) return saved;
    } catch {}
    return { subject: "The Story of the Tadpole", read: "The Year Round by C.J. Hylander · Spring section", observe: "Go outside and look near ponds or puddles for frogs or tadpoles. Just notice." };
  });
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <Icon.Leaf />
          <p className="eyebrow" style={{ marginBottom: 0 }}>Nature Study</p>
      ...
      <div style={{ marginBottom: natureDay ? 12 : 0, paddingBottom: natureDay ? 12 : 0, borderBottom: natureDay ? "1px solid var(--rule)" : "none" }}>
        <p style={{ fontFamily: "'Lato', sans-serif", fontSize: 9, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--ink-faint)", marginBottom: 3 }}>This week's topic</p>
        <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, color: "var(--ink)", marginBottom: 4 }}>{current.subject}</p>
...
function OutdoorTrackerCard({ initialMinutes, initialWeekStart, saveToMeta, userId }) {
  ...
  useEffect(() => {
    if (initialWeekStart === currentWeekStart) setMinutes(initialMinutes || 0);
  }, [initialMinutes]);
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        <div style={{ position: "relative", width: 64, height: 64, flexShrink: 0 }}>
          <svg width="64" height="64" style={{ transform: "rotate(-90deg)" }}>
            ...
            <circle cx="32" cy="32" r={r} fill="none" stroke={goalReached ? "var(--gold)" : "var(--sage)"} strokeWidth="3.5"
              strokeDasharray={dash + " " + circ} strokeLinecap="round" style={{ transition: "stroke-dasharray .5s ease" }}/>
          </svg>
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 14, color: goalReached ? "var(--gold)" : "var(--sage)", lineHeight: 1, fontWeight: 600 }}>
              {hours > 0 && <>{hours}<span style={{ fontSize: 9 }}>h</span></>}
              {mins > 0 && <>{mins}<span style={{ fontSize: 9 }}>m</span></>}
              {hours === 0 && mins === 0 && <span style={{ fontSize: 10, color: "var(--ink-faint)" }}>0h</span>}
            </span>
          </div>
        </div>
        <div style={{ flex: 1 }}>
          <p style={{ fontFamily: "'Lato', sans-serif", fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", color: goalReached ? "var(--gold)" : "var(--ink-faint)", marginBottom: 8 }}>
            {goalReached ? "Goal reached this week ✦" : "Goal: " + OUTDOOR_GOAL_HOURS + " hours · " + (OUTDOOR_GOAL_HOURS - hours) + "h remaining"}
          </p>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {[15, 30, 45, 60].map(n => (
              <button key={n} onClick={() => adjust(n)}
                style={{ background: "var(--sage-bg)", border: "1px solid var(--sage-md)", borderRadius: 2, padding: "5px 8px", fontSize: 10, fontFamily: "'Lato', sans-serif", color: "var(--sage)", cursor: "pointer" }}>
                +{n}m
              </button>
            ))}
            <button onClick={() => adjust(-15)}
              style={{ background: "none", border: "1px solid var(--rule)", borderRadius: 2, padding: "5px 8px", fontSize: 10, fontFamily: "'Lato', sans-serif", color: "var(--ink-faint)", cursor: "pointer" }}>
              -15m
            </button>
          </div>
        </div>
...
function TodaySchedule({ today, blocks, onNavigate, settings, wovenBeauty, week }) {
  ...
  const persist = (newItems, newBeauty) => {
    ...
    if (userId) {
      saveDailyState(userId, dateKey, { day: today, items: newItems, beautyDone: newBeauty !== undefined ? newBeauty : beautyDone });
    }
  };
