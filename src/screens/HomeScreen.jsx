...
// ─── TODAY’S SCHEDULE ─────────────────────────────────────────────────────────
// Storage key scoped to today’s date so it resets each morning automatically
const getScheduleKey = (today) => {
const date = new Date().toISOString().split(“T”)[0];
return `tend_schedule_${today}_${date}`;
};

function loadScheduleState(today, blocks) {
try {
const saved = localStorage.getItem(getScheduleKey(today));
if (saved) {
const parsed = JSON.parse(saved);
// Merge saved state onto current blocks (blocks may have changed)
return blocks.map(b => {
const s = parsed.find(p => p.id === b.id);
return { …b, status: s?.status || “pending”, motherNote: s?.motherNote || “”, subChecked: s?.subChecked || {} };
});
}
} catch {}
...
const [items, setItems]             = useState(() => loadScheduleState(today, blocks));
...
useEffect(() => {
try {
const toSave = items.map(({ id, status, motherNote, subChecked }) => ({ id, status, motherNote, subChecked }));
localStorage.setItem(getScheduleKey(today), JSON.stringify(toSave));
} catch {}
}, [items, today]);
