...
// ─── NATURE STUDY LOOP ────────────────────────────────────────────────────────
// Three rotating steps — Read, Walk, Journal — on Monday and Friday only.
// Loop key stored in localStorage so it picks up where you left off.
const NATURE_LOOP_KEY = "tend_nature_loop_step";
const NATURE_LOOP_STEPS = [
...
function getNatureLoopStep() {
  try { return parseInt(localStorage.getItem(NATURE_LOOP_KEY) || "0", 10) % 3; } catch { return 0; }
}
function advanceNatureLoop() {
  const next = (getNatureLoopStep() + 1) % 3;
  try { localStorage.setItem(NATURE_LOOP_KEY, String(next)); } catch {}
  return next;
}
...
function NatureOutdoorCard({ onNavigate, initialMinutes, saveToMeta, today, isPaid }) {
  ...
  return (
    <div className="card" style={{ marginBottom: 20 }}>
      ...
      {/* Current topic — compact, no duplicate */}
      {isNatureDay ? null : (
      ...
      {/* Nature loop — only on Monday and Friday */}
      {isNatureDay && (
        <div style={{ marginBottom: 12, paddingBottom: 12, borderBottom: "1px solid var(--rule)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 2 }}>
                ...
                <p style={{ fontFamily: "'Lato', sans-serif", fontSize: 9, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--sage)" }}>
                  Nature Loop · {step.step}
                </p>
              </div>
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, color: "var(--ink)", marginBottom: 1 }}>{current.subject}</p>
              <p style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", fontSize: 9, color: "var(--sage)", opacity: 0.8 }}>📖 {current.read}</p>
            </div>
            <button onClick={markDone}
              style={{ background: "var(--sage-bg)", border: "1px solid var(--sage-md)", borderRadius: 2, padding: "5px 10px", cursor: "pointer", fontSize: 9, fontFamily: "'Lato', sans-serif", letterSpacing: ".1em", textTransform: "uppercase", color: "var(--sage)", flexShrink: 0, marginLeft: 12 }}>
              Done ✦
            </button>
          </div>
          <p style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", fontSize: 13, color: "var(--ink-faint)", lineHeight: 1.6, marginBottom: 8 }}>
            {step.getInstruction(current)}
          </p>
          {/* Loop progress dots */}
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            {NATURE_LOOP_STEPS.map((s, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <div style={{ width: 7, height: 7, borderRadius: "50%", background: i === loopStep ? "var(--sage)" : "var(--rule)", transition: "background .3s" }} />
                <span style={{ fontFamily: "'Lato', sans-serif", fontSize: 8, letterSpacing: ".08em", color: i === loopStep ? "var(--sage)" : "var(--ink-faint)", textTransform: "uppercase" }}>{s.step}</span>
              </div>
            ))}
            <span style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", fontSize: 11, color: "var(--ink-faint)", marginLeft: 4 }}>pick up where you left off</span>
          </div>
        </div>
      )}
    ...
    <div className="screen">
      ...
      <div style={{ marginTop: 40, marginBottom: 32, padding: "28px 20px 24px", borderTop: "1px solid var(--rule)", textAlign: "center" }}>
        ...
        <div style={{ marginTop: 20, display: "flex", justifyContent: "center" }}>
          <img src="/DS_Primary_Logo_Transparent.png" alt="Delight & Savor" style={{ width: 64, height: 64, opacity: 0.12 }} />
        </div>
      </div>
    </div>
  );
