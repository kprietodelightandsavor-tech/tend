...
// ─── FAMILY BIBLE STUDY CARD ───────────────────────────────────────────
function FamilyBibleStudy() {
  const [, forceUpdate] = useState(0);
  const refresh = () => forceUpdate(n => n + 1);

  const handleComplete = (stream) => {
    markStreamComplete(stream);
    refresh();
  };

  const handleUndo = (stream) => {
    undoStreamComplete(stream);
    refresh();
  };

  return (
    <div style={{ background: "var(--cream)", border: "1px solid var(--sage-md)", borderRadius: 4, padding: "16px 18px", marginBottom: 24 }}>
      <p style={{ fontSize: 10, fontFamily: "'Lato', sans-serif", letterSpacing: ".12em", textTransform: "uppercase", color: "var(--sage)", marginBottom: 4 }}>
        Family Bible Study
      </p>
      <p style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", fontSize: 13, color: "var(--ink-faint)", marginBottom: 14, lineHeight: 1.5 }}>
  ...
  return (
    <div className={`screen${isSummer ? " summer" : ""}`}>
      ...
      {isSummer && isWeekend ? (
        ...
        <TodaySchedule today={today} blocks={todayBlocks} onNavigate={onNavigate} settings={settings} wovenBeauty={!isSummer} week={week} dailyOffset={dailyOffset} viewDate={viewDate} isToday={isToday} isViewOnly={isViewOnly} />
      )}
