...
import { AttentionIcon, NarrationIcon, OutdoorIcon, StillnessIcon, OrderlyIcon } from "../components/HabitScreen";
...
export function TendIcon({ size = 64 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="32" y="58" textAnchor="middle" fontFamily="'Playfair Display', Georgia, serif" fontSize="5" fill="#A9B786" fontStyle="italic" letterSpacing="0.08em">tend</text>
    </svg>
  );
}
...
export default function HabitsScreen() {
  ...
  const chooseHabit = (key) => { setActiveHabit(key); setHabitStartDate(new Date()); setReflectionDone(false); setReflectionStep(0); setReflectionAnswers({}); setView("home"); };
  ...
  if (view === "detail" && detailHabit) {
    ...
    const HIcon  = HABIT_ICON_MAP[detailHabit];
    ...
    return (
      <div className="screen">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}><HIcon /><h1 className="display serif" style={{ fontSize: 26 }}>{h.name}</h1></div>
  ...
  if (view === "library") {
    return (
      <div className="screen">
        ...
        {HABIT_KEYS.map(key => {
          const h = HABIT_PROMPTS[key], HIcon = HABIT_ICON_MAP[key], isActive = activeHabit === key;
          return (
            <button key={key} onClick={() => { setDetailHabit(key); setView("detail"); }}
              style={{ width: "100%", background: isActive ? "var(--sage-bg)" : "none", border: `1px solid ${isActive ? "var(--sage-md)" : "var(--rule)"}`, borderRadius: 3, padding: "16px 18px", marginBottom: 10, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 14 }}>
              <HIcon />
              <div style={{ flex: 1 }}>
  ...
  const ActiveIcon = HABIT_ICON_MAP[activeHabit];
  const day      = new Date().getDay();
    ...
    <div className="screen">
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          ...
          <button onClick={() => setView("library")} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, color: "var(--ink-faint)", fontFamily: "'Lato', sans-serif", letterSpacing: ".1em", textTransform: "uppercase" }}>Change</button>
        </div>
