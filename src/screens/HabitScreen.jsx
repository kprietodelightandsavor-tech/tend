...
const FREE_HABIT = "attention";

const HABIT_ICONS = {
  attention: () => (
    <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#93A388" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  stillness: () => (
    <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#B96A4B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
function HabitPaywallBanner() {
  return (
    <div style={{ borderTop: "1px solid rgba(169,183,134,.2)", paddingTop: 20, marginTop: 8, textAlign: "center" }}>
      <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 17, fontWeight: 400, color: "var(--ink)", marginBottom: 8 }}>✦ Four more habits await</p>
      <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 14, fontStyle: "italic", color: "var(--ink-faint)", lineHeight: 1.7, maxWidth: 260, margin: "0 auto 18px" }}>
        Upgrade to unlock Narration, Outdoor Life, Stillness, and Orderliness — all five habits with daily prompts and 12-week reflections.
      </p>
...
// ─── HABIT SCREEN ─────────────────────────────────────────────────────────────
export default function HabitScreen({ settings, onNavigate }) {
  ...
  const chooseHabit = (key) => {
    setActiveHabit(key); setHabitStartDate(new Date());
    setReflectionDone(false); setReflectionStep(0); setReflectionAnswers({}); setView("home");
  };
  ...
  if (view === "reflect") {
    ...
    return (
      <div className="screen">
        <button onClick={() => setView("home")} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--sage)", fontSize: 13, letterSpacing: ".08em", textTransform: "uppercase", fontFamily: "'Lato', sans-serif", marginBottom: 28, display: "flex", alignItems: "center", gap: 6 }}>← Back</button>
        ...
        <p className="corm italic" style={{ fontSize: 15, color: "var(--ink-faint)", marginBottom: 32, lineHeight: 1.7 }}>Take your time with each question. There is no right answer — only what is true for your home.</p>
        <div style={{ display: "flex", gap: 6, marginBottom: 32, justifyContent: "center" }}>
          {REFLECTION_SCAFFOLD.map((_, i) => (<div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: i <= reflectionStep ? "var(--sage)" : "var(--rule)", transition: "background .3s" }} />))}
        </div>
        ...
        <textarea className="textarea" placeholder="Write what comes to mind…" value={answer} onChange={e => setReflectionAnswers(prev => ({ ...prev, [reflectionStep]: e.target.value }))} rows={5} />
        <div style={{ display: "flex", gap: 12, marginTop: 20 }}>
          {reflectionStep > 0 && <button className="btn-ghost" onClick={() => setReflectionStep(s => s - 1)}>← Back</button>}
          <button className="btn-sage" style={{ flex: 1 }} onClick={() => { if (isLast) { setReflectionDone(true); setView("library"); } else setReflectionStep(s => s + 1); }}>
  ...
  if (view === "detail" && detailHabit) {
    ...
    const isLockedHabit = !isPaid && detailHabit !== FREE_HABIT;
    return (
      <div className="screen">
        <button onClick={() => setView("library")} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--sage)", fontSize: 13, letterSpacing: ".08em", textTransform: "uppercase", fontFamily: "'Lato', sans-serif", marginBottom: 28, display: "flex", alignItems: "center", gap: 6 }}>← Library</button>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          <HIcon />
          <h1 className="display serif" style={{ fontSize: 26 }}>{h.name}</h1>
          {isLockedHabit && <span style={{ color: "var(--ink-faint)" }}><LockIcon /></span>}
        </div>
        <p className="corm italic" style={{ fontSize: 16, color: "var(--ink-lt)", lineHeight: 1.8, marginBottom: 28 }}>{h.desc}</p>
        {isLockedHabit ? (
          <>
            <div style={{ pointerEvents: "none", userSelect: "none", maskImage: "linear-gradient(to bottom, black 30%, transparent 90%)", WebkitMaskImage: "linear-gradient(to bottom, black 30%, transparent 90%)", maxHeight: 180, overflow: "hidden", opacity: 0.45 }}>
              <p className="eyebrow" style={{ marginBottom: 16 }}>Today's Ideas</p>
              {h.daily[new Date().getDay()].slice(0, 2).map((p, i) => (
                <p key={i} className="corm italic" style={{ fontSize: 16, color: "var(--ink-lt)", lineHeight: 1.8, marginBottom: 14 }}>{p}</p>
              ))}
            </div>
            <HabitPaywallBanner />
          </>
          ...
          <>
            {isActive && <div className="card-sage" style={{ marginBottom: 24 }}><p className="eyebrow" style={{ marginBottom: 6 }}>Currently in focus</p><p className="caption italic">Week {weeksPassed + 1} of 12</p></div>}
            <p className="eyebrow" style={{ marginBottom: 16 }}>Today's Ideas</p>
            ...
            {!isActive && <button className="btn-sage" style={{ width: "100%", marginTop: 32 }} onClick={() => chooseHabit(detailHabit)}>Make This Our Focus</button>}
          </>
        )}
      </div>
    );
  ...
  if (view === "library") {
    return (
      <div className="screen">
        ...
        <p className="corm italic" style={{ fontSize: 15, color: "var(--ink-faint)", marginBottom: 28, lineHeight: 1.7 }}>Charlotte Mason taught that one habit, tended faithfully over time, changes a child far more than many habits pursued at once.</p>
        {HABIT_KEYS.map(key => {
          const h = HABIT_PROMPTS[key];
          const HIcon = HABIT_ICONS[key];
          const isActive = activeHabit === key;
          const isLockedHabit = !isPaid && key !== FREE_HABIT;
          return (
            <button key={key} onClick={() => { setDetailHabit(key); setView("detail"); }}
              style={{ width: "100%", background: isActive ? "var(--sage-bg)" : "none", border: isActive ? "1px solid var(--sage-md)" : "1px solid var(--rule)", borderRadius: 3, padding: "16px 18px", marginBottom: 10, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 14, opacity: isLockedHabit ? 0.65 : 1 }}>
              <HIcon />
              <div style={{ flex: 1 }}>
              ...
              {isLockedHabit && <span style={{ color: "var(--ink-faint)", flexShrink: 0 }}><LockIcon /></span>}
            </button>
          );
        })}
        {!isPaid && <HabitPaywallBanner />}
      </div>
    );
  }
  ...
  return (
    <div className="screen">
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 28 }}>
        <TendIcon size={52} />
        <div><p className="eyebrow" style={{ marginBottom: 4 }}>Habit Formation</p><h1 className="display-sm serif">The Tended Life</h1></div>
      </div>
      {showReassessPrompt && (
        <div className="card-gold" style={{ marginBottom: 24 }}>
          <p className="eyebrow" style={{ marginBottom: 8 }}>Twelve Weeks</p>
          <p className="corm italic" style={{ fontSize: 16, color: "var(--ink-lt)", lineHeight: 1.8, marginBottom: 16 }}>Your family has been tending <em>{active.name}</em> for twelve weeks. It may be time to pause and reflect before choosing what to tend next.</p>
          <button className="btn-gold" style={{ width: "100%" }} onClick={() => setView("reflect")}>Begin Reflection →</button>
        </div>
      )}
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}><ActiveIcon /><p className="eyebrow" style={{ marginBottom: 0 }}>This Season's Habit</p></div>
          <button onClick={() => setView("library")} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, color: "var(--ink-faint)", fontFamily: "'Lato', sans-serif", letterSpacing: ".1em", textTransform: "uppercase" }}>
            {isPaid ? "Change" : "View all →"}
          </button>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span className="caption">Week {Math.min(weeksPassed + 1, 12)} of 12</span><span className="caption">{Math.round(Math.min(weeksPassed / 12, 1) * 100)}%</span></div>
          <div className="progress-track"><div className="progress-fill" style={{ width: `${Math.min((weeksPassed / 12) * 100, 100)}%` }} /></div>
        </div>
        <div className="rule" />
        <p className="eyebrow" style={{ marginBottom: 16 }}>Today's Ideas</p>
      ...
      <div className="card-gold" style={{ marginTop: 4 }}>
        <p className="corm italic" style={{ fontSize: 16, color: "var(--ink-lt)", lineHeight: 1.8 }}>"Habit is ten natures."</p>
        <p className="caption" style={{ marginTop: 8 }}>— Charlotte Mason</p>
      </div>
      <button className="btn-ghost" style={{ width: "100%", marginTop: 20 }} onClick={() => setView("library")}>Explore All Five Habits</button>
    </div>
  );
}
